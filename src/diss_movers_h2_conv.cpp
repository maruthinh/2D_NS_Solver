...
Diss_MOVERS_H2_Conv(int ib, int id1, int id2, int jb, int jd1, int jd2, double ***&cv, double ***&dv, double ***&si,
    ...
    double ar, al, max_eig, min_eig, phi, dx, dy;
    double *fd, *fr, *fl, *Ur, *Ul, *deltl, *deltr, *mdiss;
    double const beta = 0.5, eps = 1e-6;
    //double area_avg;
    ...
    for (int j = 2; j <= jb; j++) {
        for (int i = 2; i <= id1; i++) {
            ...
            mdiss[0] = (Movers(fr[0], fl[0], Ur[0], Ul[0], max_eig, min_eig) + beta * phi * max_eig);
            mdiss[1] = (Movers(fr[1], fl[1], Ur[1], Ul[1], max_eig, min_eig) + beta * phi * max_eig);
            mdiss[2] = (Movers(fr[2], fl[2], Ur[2], Ul[2], max_eig, min_eig) + beta * phi * max_eig);
            mdiss[3] = (Movers(fr[3], fl[3], Ur[3], Ul[3], max_eig, min_eig) + beta * phi * max_eig);
            ...
            fd[0] = 0.5 * fabs(mdiss[0]) * (Ur[0] - Ul[0]);
            fd[1] = 0.5 * fabs(mdiss[1]) * (Ur[1] - Ul[1]);
            fd[2] = 0.5 * fabs(mdiss[2]) * (Ur[2] - Ul[2]);
            fd[3] = 0.5 * fabs(mdiss[3]) * (Ur[3] - Ul[3]);
    ...
    for (int i = 2; i <= ib; i++) {
        for (int j = 2; j <= jd1; j++) {
            ...
            deltl[3] = 0.5 * dy * Minmod_Slope(duj[3][i][j], duj[3][i][jm1], dy);
            deltr[3] = 0.5 * dy * Minmod_Slope(duj[3][i][jp1], duj[3][i][j], dy);
