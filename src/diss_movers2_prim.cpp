...
void Diss_MOVERS2_Prim(int ib, int id1, int id2, int jb, int jd1, int jd2, double ***&cv, double ***&dv, double ***&si,
    ...
    for (int j = 2; j <= jb; j++) {
        for (int i = 2; i <= id1; i++) {
            ...
            fd[0] = 0.5 * Movers(fr[3], fl[3], Ur[3], Ul[3], max_eig, min_eig) * (Ur[0] - Ul[0]);
            fd[1] = 0.5 * Movers(fr[3], fl[3], Ur[3], Ul[3], max_eig, min_eig) * (Ur[1] - Ul[1]);
            fd[2] = 0.5 * Movers(fr[3], fl[3], Ur[3], Ul[3], max_eig, min_eig) * (Ur[2] - Ul[2]);
            fd[3] = 0.5 * Movers(fr[3], fl[3], Ur[3], Ul[3], max_eig, min_eig) * (Ur[3] - Ul[3]);
...
T Movers(T Fr, T Fl, T Ur,T Ul,T L_max, T L_min){
	...
	double S, delta, k=0.5;

        delta = k*L_max;    
        
	if (fabs(Ur-Ul)!=0) S=fabs(((Fr-Fl)/(Ur-Ul)));
	else S=L_min;
	
	if (fabs(S)>=L_max) S = L_max;
	else if(fabs(S)<=L_min) S = L_min;
	
        if(S<delta) return (S*S+delta*delta)/(2*delta);
        else return S;
...
T Movers(T Fr, T Fl, T Ur, T Ul, T L_max, T L_min) {
    ...
    if (fabs(Fr - Fl) <= 10e-6 && fabs(Ur - Ul) <= 10e-6) S = L_min;
    else if (fabs(Fr - Fl) <= 10e-6 && fabs(Ur - Ul) > 10e-6) S = fabs(((Fr - Fl) / (Ur - Ul)));
    else if (fabs(Fr - Fl) > 10e-6 && fabs(Ur - Ul) <= 10e-6) S = L_min;

    if (fabs(Fr - Fl) > 10e-6 && fabs(Ur - Ul) > 10e-6) S1 = fabs(((Fr - Fl) / (Ur - Ul)));
