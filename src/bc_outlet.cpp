...
void BC_Outlet(int bind, int bnode, int sbind, int ebind, double ***&si, double ***&sj, double ***&cv,
    ...
    double rhoe, ue, ve, pe, qqe, qq2e, qn, crho0, rhoa, ua, va, pa, pb, ul, vl, pl, sgn, cosa, co2, rinv, dis, cb, cco2, Tb, rhob, uabsb, ub, vb, Mach;
    maxwchg = 0.0;
    double rhoinv, rrhoc;
...
/**computation of values at the boundary**/
    ...
    if(bind == 1 or bind == 3){
        for (int i = sbind; i <= ebind; i++){
            ...
            ve = dv[2][i][ins1];
            qqe = ue*ue + ve*ve;
            qq2e = sqrt(ue*ue + ve*ve);
            pe = dv[3][i][ins1];
            vnor = ue*nx + ue*ny;
            Mach = qq2e/dv[5][i][ins1];

            if(Mach<1.0){
                rrhoc = rhoinv/dv[5][i][ins1];
            }
            co2 = dv[5][i][ins1]*dv[5][i][ins1] + (GammaMinus1)*0.2*qqe;
            rinv = vnor - ((2*dv[5][i][ins1])/GammaMinus1);
            dis  = (GammaMinus1*cosa*cosa+2.0)*co2/(GammaMinus1*rinv*rinv) - 0.5*GammaMinus1;
            cb   = -rinv*(GammaMinus1/(GammaMinus1*cosa*cosa+2.0))*(1.0+cosa*sqrt(dis));
            cco2 = std::min(cb*cb/co2, 1.0);
            Tb = tinf*cco2;
            pb = pinf*pow((Tb/tinf), Gamma/GammaMinus1);
            rhob = pb/(Rgas*Tb);
            uabsb = sqrt(2.0*Cp*(tinf-Tb));
            ub = uabsb*cos(alpha);
            vb = uabsb*sin(alpha); 
    ...
    if (bind == 2 or bind == 4) {
        for (int j = sbind; j <= ebind; j++){
            ...
            co2 = dv[5][ins1][j]*dv[5][ins1][j] + (GammaMinus1)*0.2*qqe;
            rinv = vnor - ((2*dv[5][ins1][j])/GammaMinus1);
            dis  = (GammaMinus1*cosa*cosa+2.0)*co2/(GammaMinus1*rinv*rinv) - 0.5*GammaMinus1;
            cb   = -rinv*(GammaMinus1/(GammaMinus1*cosa*cosa+2.0))*(1.0+cosa*sqrt(dis));
            cco2 = std::min(cb*cb/co2, 1.0);
            Tb = tinf*cco2;
