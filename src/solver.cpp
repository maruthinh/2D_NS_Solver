    ...
    case 0:
        ...
        for (int j = 2; j <= jb; j++)
        {
            for (int i = 2; i <= ib; i++)
            {
                ...
                {
                    std::cout << " In solver_explicit: the values of conserved var at i=" << i << "\t"
                              << "j=" << j << "\t"
        ...
        for (int k = 0; k < 2; k++)
        {
            ...
            for (int j = 2; j <= jb; j++)
            {
                for (int i = 2; i <= ib; i++)
                {
                    ...
                    if ((cv[0][i][j] != cv[0][i][j]) or (cv[1][i][j] != cv[1][i][j]) or (cv[2][i][j] != cv[2][i][j]) or (cv[3][i][j] != cv[3][i][j]) or pressure < 0.0)
                    {
