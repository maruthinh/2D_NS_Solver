...
void Diss_MOVERS_LE1(int ib, int id1, int id2, int jb, int jd1, int jd2, double ***&cv, double ***&dv, double ***&si,
    ...
    for (int i = 2; i <= ib; i++) {
        for (int j = 2; j <= jd1; j++) {
            ...
            fd[0] = 0.5 * fabs(mdiss[0] +
                               MinMod(cuj[0][i][jp1], cuj[0][i][j], cuj[0][i][jm1]) * (std::max(Vnr, Vnl) - mdiss[0])) *
                    (Ur[0] - Ul[0]);
            fd[1] = 0.5 * fabs(mdiss[1] +
                               MinMod(cuj[1][i][jp1], cuj[1][i][j], cuj[1][i][jm1]) * (std::max(Vnr, Vnl) - mdiss[1])) *
                    (Ur[1] - Ul[1]);
            fd[2] = 0.5 * fabs(mdiss[2] +
                               MinMod(cuj[2][i][jp1], cuj[2][i][j], cuj[2][i][jm1]) * (std::max(Vnr, Vnl) - mdiss[2])) *
                    (Ur[2] - Ul[2]);
            fd[3] = 0.5 * fabs(mdiss[3] +
                               MinMod(cuj[3][i][jp1], cuj[3][i][j], cuj[3][i][jm1]) * (std::max(Vnr, Vnl) - mdiss[3])) *
                    (Ur[3] - Ul[3]);
