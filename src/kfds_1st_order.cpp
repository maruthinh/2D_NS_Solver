//
// Created by Maruthi NH on 18-06-2018.
//
...
void KFDS_1stOrder(int id2, int jd2, int id1, int jd1, int ib, int jb, double ***&cv, double ***&dv,
    ...
    for (int j = 2; j <= jb; j++) {
        for (int i = 2; i <= id1; i++) {
            ...
            for(int k=0;k<nconv; k++){
                fd[k] = 0.5 * Movers(iFluxDiff[k][i][j], iConVarDiff[k][i][j], max_eig, min_eig) *
                                                                        (iConVarDiff[k][i][j]);

                //diss[k][i][j] = 0.5 * max_eig * (iConVarDiff[k][i][j]);
                diss[k][i][j] = diss[k][i][j] - fd[k] * ds;
                diss[k][im1][j] = diss[k][im1][j] + fd[k] * ds;

                //irhs[k][i][j] = iAvgFlux[k][i][j]-diss[k][i][j];
                //irhs[k][i-1][j] = irhs[k][i-1][j]-iAvgFlux[k][i][j]+diss[k][i][j];
    ...
    /*for(int k=0;k<nconv; k++){
        for (int j = 2; j <= jb; j++) {
            for (int i = 2; i <= ib; i++) {
                rhs[k][i][j] = rhs[k][i][j]-rhs[k][i-1][j];
            }
        }
    }*/
    ...
    for (int i = 2; i <= ib; i++) {
        for (int j = 2; j <= jd1; j++) {
            ...
            for(int k=0;k<nconv; k++){
                ...
                //diss[k][i][j] = 0.5 * max_eig * (jConVarDiff[k][i][j]);
                diss[k][i][j] = diss[k][i][j] - fd[k] * ds;
                diss[k][i][jm1] = diss[k][i][jm1] + fd[k] * ds;

                //jrhs[k][i][j] = jAvgFlux[k][i][j] - diss[k][i][j];
                //jrhs[k][i][j-1] = jrhs[k][i][j-1]-jAvgFlux[k][i][j] + diss[k][i][j];
                /*rhs[2][i][j] = rhs[2][i][j] + fc[2];
                rhs[3][i][j] = rhs[3][i][j] + fc[3];*/
...
/*    for(int k=0;k<nconv; k++){
        for (int j = 2; j <= jb; j++) {
            for (int i = 2; i <= ib; i++) {
                rhs[k][i][j] = (irhs[k][i][j]+irhs[k][i-1][j])+(jrhs[k][i][j]+jrhs[k][i][j-1]);
                //rhs[k][i][j] = irhs[k][i][j]+jrhs[k][i][j];
            }
        }
    }*/
