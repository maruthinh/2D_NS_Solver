...
void Read2DEulerRiemannProbInput(const std::string& RiemannSolverInputFile){
    ...
    infile.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    if(restart==0){
        std::cout<<"restart = 0 and solution will start from the beginning="<<cfl<<std::endl;
    }
    else{
        std::cout<<"restart = q and solution will start from the chosen iteration or time="<<cfl<<std::endl;
    }
